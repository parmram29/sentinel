...
import { Shield, ShieldAlert, ShieldCheck, Terminal, Code, Server, Lock, AlertTriangle, CheckCircle, Loader2, Upload, Download, FileText, ChevronRight, Play } from 'lucide-react';
...
export default function App() {
  ...
  // Payload State: Holds the code/logs pasted or uploaded by the user
  const [inputCode, setInputCode] = useState('');
  
  // Audit State: Tracks loading status, results, and errors for the AI audit
  const [isAuditing, setIsAuditing] = useState(false);
  const [auditResult, setAuditResult] = useState<AuditResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  
  // Ref for the hidden file input element used for uploading files
  const fileInputRef = useRef<HTMLInputElement>(null);
  
  // CLI State: Manages the active tab, command input, and execution results
  ...
  // ---------------------------------------------------------------------------
  // HIDDEN FEATURE: Developer Mode State
  // Clicking the shield icon 5 times unlocks a hidden developer panel
  // ---------------------------------------------------------------------------
  const [devClicks, setDevClicks] = useState(0);
  const [isDevMode, setIsDevMode] = useState(false);

  // Handles the easter egg click logic
  ...
  // ---------------------------------------------------------------------------
  // FILE UPLOAD HANDLER
  // Reads multiple files and appends their contents to the inputCode state
  // ---------------------------------------------------------------------------
  ...
  const handleExport = () => {
    exportReport(auditResult);
  };

  // ---------------------------------------------------------------------------
  // AUDIT EXECUTION
  // Sends the payload to the backend for AI analysis
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // CLI EXECUTION
  // Sends the shell command to the backend to be executed on the container
  // ---------------------------------------------------------------------------
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-sans selection:bg-emerald-500/30 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Shield 
              ...
              title="Sentinel Core"
            />
            {/* 🛠️ EDIT HERE: Customize your app's branding, title, and version */}
          ...
          <div className="flex items-center gap-4 text-xs font-mono text-zinc-500">
            ...
            <div className="flex items-center gap-1.5 text-emerald-500/80 md:ml-2 md:border-l border-zinc-700 md:pl-4">
              <ShieldAlert className="w-3.5 h-3.5" />
              <span className="hidden sm:inline-block">Anomaly Detection Active</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 h-full min-h-0">
        ...
        <div className="flex flex-col gap-4 h-full min-h-[400px]">
          {/* 🕵️‍♂️ HIDDEN FEATURE: Developer Panel */}
          {isDevMode && (
            <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-4 mb-2 animate-in fade-in slide-in-from-top-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-purple-400 font-mono text-xs uppercase tracking-widest font-bold flex items-center gap-2">
                  ...
                  Corporate Dev Mode Unlocked
                </h3>
                <button 
                  onClick={() => { setIsDevMode(false); setDevClicks(0); }}
              ...
              <p className="text-zinc-400 text-xs mb-3">
                Welcome to the hidden collaboration panel. The AI logic has been securely moved to the backend to prevent prompt exposure.
              </p>
              <div className="flex gap-2">
                <button 
                  onClick={() => setInputCode(prev => prev + '\n// [DEV-INJECT]: Bypass standard validation check\n')}
                ...
                <button 
                  onClick={() => console.log("Current Audit State:", auditResult)}
            ...
            {activeTab === 'payload' && (
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="hidden sm:inline-block">Upload Files</span>
                </button>
                <button
                  onClick={runAudit}
                  disabled={isAuditing || !inputCode.trim()}
          ...
          {activeTab === 'payload' ? (
            <div className="flex-1 relative rounded-lg border border-zinc-800 bg-zinc-900/50 overflow-hidden focus-within:border-emerald-500/50 transition-colors">
              <textarea
                ...
                placeholder="Paste source code, IaC (Terraform/Bicep), configuration files, or operational logs here for analysis...&#10;&#10;Or click 'Upload Files' to load multiple files at once."
                className="w-full h-full p-4 bg-transparent text-zinc-300 font-mono text-sm resize-none focus:outline-none"
                spellCheck={false}
              />
            </div>
          ) : (
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 relative rounded-lg border border-zinc-800 bg-zinc-900/50 overflow-hidden focus-within:border-emerald-500/50 transition-colors flex items-center px-3">
                  ...
                  <input
                    ...
                    placeholder="Enter CLI command (e.g., npm audit, ls -la)..."
                ...
                <button
                  ...
                  disabled={isExecutingCli || !cliCommand.trim()}
                ...
                >
                  {isExecutingCli ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                  Execute
                </button>
              </div>
              ...
              <div className="flex-1 rounded-lg border border-zinc-800 bg-black overflow-hidden flex flex-col">
                <div className="px-4 py-2 border-b border-zinc-800 bg-zinc-900/50 flex items-center justify-between">
                  ...
                  {cliOutput && (
                    <button 
                      onClick={() => {
                        const outputStr = `Command: ${cliCommand}\n\nSTDOUT:\n${cliOutput.stdout}\n\nSTDERR:\n${cliOutput.stderr}\n\nERROR:\n${cliOutput.error || 'None'}`;
                ...
                <div className="flex-1 p-4 overflow-y-auto font-mono text-xs whitespace-pre-wrap">
                  {!cliOutput && !isExecutingCli && (
                    <span className="text-zinc-600">Ready for command execution...</span>
                  )}
                  {isExecutingCli && (
                    <span className="text-emerald-500/70 animate-pulse">Executing command...</span>
                  )}
                  {cliOutput && (
                    <div className="flex flex-col gap-4">
                      {cliOutput.stdout && (
                        <div>
                          <span className="text-zinc-500 select-none">stdout &gt;</span>
                          <div className="text-zinc-300 mt-1">{cliOutput.stdout}</div>
                        </div>
                      )}
                      {cliOutput.stderr && (
                        <div>
                          <span className="text-amber-500/70 select-none">stderr &gt;</span>
                          <div className="text-amber-400 mt-1">{cliOutput.stderr}</div>
                        </div>
                      )}
                      {cliOutput.error && (
                        <div>
                          <span className="text-red-500/70 select-none">error &gt;</span>
                          <div className="text-red-400 mt-1">{cliOutput.error}</div>
                        </div>
                      )}
                      {!cliOutput.stdout && !cliOutput.stderr && !cliOutput.error && (
                        <span className="text-zinc-500 italic">Command executed successfully with no output.</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            {auditResult && (
              ...
              >
                ...
                <span className="hidden sm:inline-block">Export Sheet</span>
              </button>
            )}
          </div>
          ...
          <div className="flex-1 rounded-lg border border-zinc-800 bg-zinc-900/50 overflow-y-auto p-4 flex flex-col gap-6">
            {!auditResult && !isAuditing && !error && (
              <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 gap-3 min-h-[200px]">
                ...
                <p className="font-mono text-sm text-center px-4">System idle. Awaiting payload or file upload for analysis.</p>
              </div>
            )}
            ...
            {isAuditing && (
              <div className="flex-1 flex flex-col items-center justify-center text-emerald-500/70 gap-4 min-h-[200px]">
                ...
                <div className="font-mono text-xs uppercase tracking-widest animate-pulse text-center">
                  Analyzing against CompTIA standards...
                </div>
              </div>
            )}
            ...
            {error && (
              <div className="p-4 rounded border border-red-500/20 bg-red-500/10 text-red-400 font-mono text-sm flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold mb-1">Audit Failure</div>
                  {error}
                </div>
              </div>
            )}
              ...
              <>
                ...
                {auditResult.isSecure ? (
                  <div className="p-4 rounded border border-emerald-500/20 bg-emerald-500/10 text-emerald-400 font-mono text-sm flex items-center gap-3">
                    ...
                    <span>Scan complete: No violations of Security+ or Cloud+ standards detected in this snippet.</span>
                  </div>
                ) : (
                ...
                {/* Findings List (Cards instead of Table for better highlighting) */}
                      ...
                      return (
                        <div key={idx} className="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden flex flex-col">
                          <div className="p-4 border-b border-zinc-800/50 bg-zinc-900/30 flex flex-wrap items-start justify-between gap-3">
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded text-[10px] font-mono uppercase tracking-wider border ${severityColor}`}>
                              {finding.severity}
                            </span>
                          </div>
                          ...
                          <div className="p-4 flex flex-col gap-4">
                            ...
                            {finding.affectedCode && (
                              <div className="flex flex-col gap-1.5">
                                ...
                                <pre className={`p-3 rounded border font-mono text-xs overflow-x-auto ${highlightColor}`}>
                            ...
                            {/* Step-by-Step Fix (Advanced Sheet) */}
                            {finding.detailedSteps && finding.detailedSteps.length > 0 && (
                              <div className="mt-2 p-4 rounded bg-zinc-900 border border-zinc-800">
                                <h4 className="text-xs font-mono uppercase tracking-wider text-emerald-500 flex items-center gap-2 mb-3">
                                  ...
                                  Step-by-Step Remediation Sheet
                                </h4>
                                <ol className="space-y-2">
                                  {finding.detailedSteps.map((step, stepIdx) => (
                                    <li key={stepIdx} className="text-sm text-zinc-300 flex items-start gap-2">
                                      <span className="text-emerald-500/50 font-mono text-xs mt-0.5">{stepIdx + 1}.</span>
