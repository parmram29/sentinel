...
function strike(ip: string, reason: string, rid: string) {
  ...
  strikeMap.set(ip, n);
  siem("HIGH", "STRIKE", ip, rid, {
    reason,
    strikes: n,
    threshold: STRIKE_LIMIT,
  });
  if (n >= STRIKE_LIMIT)
    blockIP(ip, `Auto-blocked: ${reason} (${n} strikes)`, rid);
}

function isBlocked(ip: string): boolean {
  const b = blockedIPs.get(ip);
  if (!b) return false;
  if (Date.now() - b.at > BLOCK_TTL) {
...
async function crowdSecCheck(ip: string, rid: string): Promise<boolean> {
  ...
  if (cached && Date.now() - cached.at < CROWDSEC_CACHE_TTL)
  ...
  try {
    ...
    const blocked =
      data.reputation === "malicious" || data.reputation === "suspicious";
    crowdSecCache.set(ip, { blocked, at: Date.now() });
    if (blocked)
      siem("HIGH", "CROWDSEC_BLOCK", ip, rid, {
...
function getRid(req: Request): string {
  return (req as any).rid ?? "no-rid";
}
...
async function ipGuard(req: Request, res: Response, next: NextFunction) {
  ...
  if (isBlocked(ip)) {
    siem("WARN", "BLOCKED_IP_HIT", ip, rid, {
      path: req.path,
      method: req.method,
    });
...
// ============================================================
const SCANNER_UA =
  /sqlmap|nikto|nmap|masscan|dirsearch|gobuster|wfuzz|burpsuite|zgrab|nuclei|python-requests\/2\.[01]/i;
...
const THREAT_SIGNATURES: { label: string; pattern: RegExp }[] = [
  {
    label: "SQLi",
    pattern: /(\bUNION\b.{0,20}\bSELECT\b|\bDROP\b.{0,10}\bTABLE\b)/i,
  },
  { label: "XSS", pattern: /<script[\s\S]*?>[\s\S]*?<\/script>/i },
  { label: "PathTraversal", pattern: /\.\.[/\\]/ },
  {
    label: "TemplateInjection",
    pattern: /\$\{[\s\S]{0,80}\}|#\{[\s\S]{0,80}\}/,
  },
  { label: "SSRF", pattern: /(wget|curl)\s+https?:\/\//i },
  {
    label: "CodeInjection",
    pattern: /\b(eval|exec|system|passthru|shell_exec)\s*\(/i,
  },
...
function scanForThreats(payload: string): string[] {
  return THREAT_SIGNATURES.filter(({ pattern }) => pattern.test(payload)).map(
    ({ label }) => label,
  );
}
...
function circuitFailure() {
  ...
  if (circuit.failures >= CB_THRESHOLD) {
    circuit.state = "OPEN";
    console.error(
      "[Sentinel] ⚡ Circuit OPEN — Claude API degraded. Auto-recovery in 30s.",
    );
  }
}
...
async function startServer() {
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (inputCode.length > 500_000)
          return res
            .status(413)
            .json({ error: "Payload exceeds 500,000 character limit." });

        const threats = scanForThreats(inputCode);
        if (threats.length > 0)
          siem("WARN", "SUSPICIOUS_PAYLOAD", ip, rid, {
            threats,
            payloadLength: inputCode.length,
          });
        ...
        if (!circuitAllow()) {
          ...
          return res
            ...
            .json({
              error:
                "Audit service temporarily unavailable. Please retry in 30 seconds.",
            });
        }

        const apiKey = process.env.ANTHROPIC_API_KEY;
        if (!apiKey)
          return res.status(503).json({ error: "Audit service unavailable." });
        ...
        try {
          response = await client.messages.create({
            ...
            messages: [
              {
                role: "user",
                content: `Audit the following payload:\n\n${inputCode}`,
              },
            ],
            tools: [
              {
                name: "audit_report",
                description: "Return the structured security audit report.",
                input_schema: AUDIT_RESPONSE_SCHEMA as any,
              },
            ],
        ...
        if (!toolUseBlock || toolUseBlock.type !== "tool_use")
          return res
            .status(502)
            .json({ error: "Invalid response from audit model." });

        siem("INFO", "AUDIT_COMPLETE", ip, rid, {
          payloadLength: inputCode.length,
          threats,
        ...
        res
          .status(500)
          .json({ error: "An internal error occurred during the audit." });
      }
    },
  );
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!["http:", "https:"].includes(parsed.protocol))
          return res
            .status(400)
            .json({ error: "Only http/https URLs allowed." });
        ...
        try {
          const r = await fetch(url, {
            ...
            headers: { "User-Agent": "Sentinel-Lite/2.0 Security-Auditor" },
          });
          clearTimeout(timeout);
          if (!r.ok)
            return res.status(502).json({ error: `URL returned ${r.status}.` });
        ...
        const scriptSrcs = srcMatches
          .map((m) => m[1])
          .filter((s) => !s.includes("cdn") && !s.includes("analytics"))
          .slice(0, 5);
        ...
        for (const src of scriptSrcs.slice(0, 3)) {
          try {
            ...
            const r = await fetch(fullSrc, {
              headers: { "User-Agent": "Sentinel-Lite/2.0" },
            });
            if (r.ok)
              fetchedScripts.push(
        ...
        res.json({
          content,
          scriptCount: scriptMatches.length + fetchedScripts.length,
          url,
        });
      } catch (err: any) {
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!isCommandAllowed(command)) {
          ...
          siem("HIGH", "COMMAND_BLOCKED", ip, rid, {
          ...
          return res
            ...
            .json({
              error:
                "Command not permitted. Only security audit tools are allowed.",
            });
        }

        siem("INFO", "COMMAND_EXEC", ip, rid, {
          command,
          user: (req as any).user?.email,
        });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      const signature = req.headers["x-hub-signature-256"] as
        ...
        | undefined;
      if (!signature) {
        strike(ip, "Webhook missing signature", rid);
      ...
      if (event !== "push")
        return res
          .status(200)
          .json({ message: `Event '${event}' acknowledged.` });
      ...
      for (const commit of payload.commits ?? [])
        [...(commit.added ?? []), ...(commit.modified ?? [])].forEach(
          (f: string) => changedFiles.add(f),
        );

      const filesToAudit = [...changedFiles]
        .filter((f) =>
          /\.(ts|tsx|js|jsx|py|go|rb|java|cs|php|tf|yaml|yml|json|toml|sh)$/i.test(
            f,
          ),
        )
        .slice(0, 10);
      ...
      for (const file of filesToAudit) {
        try {
          const r = await fetch(
            `https://raw.githubusercontent.com/${repo}/${commitSha}/${file}`,
            { headers },
          );
          if (r.ok)
            fileContents.push(
      ...
      try {
        const { SYSTEM_INSTRUCTION, AUDIT_RESPONSE_SCHEMA } =
        ...
        const response = await client.messages.create({
          ...
          messages: [
            {
              role: "user",
              content: `Audit push from ${pusher} on ${repo}:${branch}:\n\n${fileContents.join("\n\n")}`,
            },
          ],
          tools: [
            {
              name: "audit_report",
              description: "Return structured audit report.",
              input_schema: AUDIT_RESPONSE_SCHEMA as any,
            },
          ],
          tool_choice: { type: "tool", name: "audit_report" },
        ...
        if (toolBlock?.type === "tool_use") {
          ...
          const status = result.isSecure
            ? "PASS"
            : `FAIL (${result.findings?.length} findings)`;
          siem("INFO", "WEBHOOK_AUDIT_COMPLETE", ip, rid, {
            repo,
            branch,
            status,
          });
  ...
  app.get("/api/security/events", (req: Request, res: Response) => {
    const passphrase = req.headers["x-sentinel-passphrase"] as
      | string
      | undefined;
    ...
    const events = severity
      ? eventLog
          .filter((e) => e.severity === severity.toUpperCase())
          .slice(-limit)
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `[Sentinel] Auth            : Supabase JWT verification active`,
    );
    console.log(
      `[Sentinel] CrowdSec        : ${process.env.CROWDSEC_API_KEY ? "active" : "disabled (no API key)"}`,
    );
    console.log(
      `[Sentinel] Rate limits     : 10 audits/min · 20 exec/min · 30 webhook/min`,
    );
    console.log(
      `[Sentinel] Honeypot paths  : ${HONEYPOT_PATHS.size} traps armed`,
    );
    console.log(
      `[Sentinel] Threat sigs     : ${THREAT_SIGNATURES.length} patterns loaded`,
    );
    console.log(
      `[Sentinel] Circuit breaker : CLOSED (threshold: ${CB_THRESHOLD} failures)`,
    );
    console.log(
      `[Sentinel] SIEM            : active (max ${MAX_LOG} events in-memory)`,
    );
  });
}
